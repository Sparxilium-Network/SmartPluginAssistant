...
* **多平台支援與來源連結**：
  * 分別串接 Modrinth (`v2` API)、PaperMC Hangar (`v1` API)、Voxel.shop (Polymart `v1` API) 與 SpigotMC (SpiGet `v2` API) 四大主流市場。
  * 標題旁皆整合專屬 API 來源外連按鈕（`API BY MODRINTH` / `API BY HANGAR` / `API BY VOXEL.SHOP` / `API BY SPIGET`），點選可直接呼叫系統預設瀏覽器訪問官方網站。
  * Voxel.shop 與 SpigotMC 模組支援免費資源一鍵直配下載與 Hash 寫入，付費/外部外掛清楚標示價格並引導至官方購買頁面。
